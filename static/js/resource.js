...
function toResourceUrl(data, channelId) {
  ...
  switch (jumpType) {
    ...
    case "70" : // 好价 清单
      obj.self = true;
      if (data.jumpCode == 'goodPrice' && data.newJump) {
        obj.path = "/goodPriceDetail/" + data.newJump + "/0"
      } else if (data.jumpCode == 'manifest' && data.newJump) {
        obj.path = "/repertoire/detail?id=" + data.newJump
      }
      break;
    default : // 默认微信首页
  ...
  switch (actionName) {
    ...
    case 'BLAdvertResourceController':
      ...
      if (resource.path) {
        ...
        if (bl_mmc) {
          if (resource.path.indexOf('?') !== -1) {
            resource.path += '&bl_mmc=' + bl_mmc
          } else {
            resource.path += '?bl_mmc=' + bl_mmc
          }
        } else if (cm_mmc) {
          if (resource.path.indexOf('?') !== -1) {
            resource.path += '&cm_mmc=' + cm_mmc
          } else {
            resource.path += '?cm_mmc=' + cm_mmc
          }
        }
    ...
    case 'H5BLShareParams':
      ...
      if (useragent.indexOf('micromessenger') >= 0) {
        if (data.shareTip) {
          ...
          shareHtml = "<div class='overlay bl_weixin_share'></div> <img class='shareTip bl_weixin_share' src='static/img/share.png'>"
        }
        $("body").append(shareHtml);
        if (data.showTip == false) {
          $(".shareTip").hide()
    ...
    case 'PresentLoginViewController':
      window.$login(data.backUrl, data.pushUrl);
      break;
      // 打开默认webView
    ...
    case 'getDesDevice':
      ...
      // 拨打电话
    case 'callPhone':
      window.location = 'tel:' + data.phoneNumber
      break;
      // 未实名认证去认证
    case 'NormalRealNameAuthenViewController':
      if (data.pageId == 'authenticate') {
        location.href = h5BaseUrl + '/member/view_autonymPass.html'
      }
      break;
      // TODO 定位
    case 'getLocation':
      ...
      break;
      // 跳门店活动报名详情页
    case 'BLVipActivityDispalyInDetailViewController':
      location.href = h5BaseUrl + '/member/getMemberActiveDetail.html?benefitCode=' + data.activityCode
      break;
      // 发票抬头管理
    case 'BLInvoiceTitleViewController':
      location.href = h5VueBaseUrl + '/userCenter/invoiceList'
      break;
      // 地址列表 实名认证
    case 'NavigateWithStringParams':
      if (data.pageId == 'userAddress') {
        location.href = h5BaseUrl + '/member/view_member_reciver_list.html'
      } else if (data.pageId == 'authenticate') {
        location.href = h5VueBaseUrl + '/userCenter/realName'
        // if (data.realNameAuthType >= 2) {
        //   location.href = h5BaseUrl + '/member/view_autonymPass.html'
        // } else {
        //   location.href = h5BaseUrl + '/member/view_setAutonym.html'
        // }
      } else if (data.pageId == 'couponEcard') {
        location.href = h5BaseUrl + '/member/view_myCouponsAndCard.html?selFlag=1'
      }
      break;
      // 我的优惠券
    case 'BLMyCouponViewController':
      location.href = h5BaseUrl + '/member/view_myCouponsAndCard.html?selFlag=1'
      break;
      // “去使用”优惠券
    case 'IBLActivityList':
      location.href = h5BaseUrl + '/promotionSearch/productList.html?couponTemplateId=' + data.ruleNo + '&flag=0'
    default:
      // TODO default
  }
};
...
bridge.prototype.LoadAPI = function(apiName, data, callback) {
  ...
  switch (apiName) {
    ...
    case 'BLDJAddCartAPIManager':
      break;
    // 薛天玲 推出登录
    case 'BLLogoutAPIManager':
      localStorage.removeItem('userInfo')
      location.replace(h5BaseUrl + '/page/view_Index.index#')
      break;
  }
};
...
/* 判断是否是百联APP 是否初始化CTJSBridge */
var ua = window.navigator.userAgent.toLowerCase();
if (ua.match(/iBailian/i) != 'ibailian') {
